...
class Custom_raw_env(SimpleEnv, EzPickle):
    def __init__(
        ...
        world_size = 2.5, # Ronchy添加 world_size参数, 地图大小: [-world_size , world_size]
    ):
        EzPickle.__init__(
            self,
            num_good=num_good,
...
class Scenario(BaseScenario):
    ...
    def global_reward(self, world):
        ...
        # 获取全局设置的围捕阈值
        capture_threshold = world.env.capture_threshold if hasattr(world, 'env') else world.world_size * 0.2
        ...
        for agent in agents:
            ...
            for adv in adversaries:
                # 计算碰撞奖励
                collision_reward = 1.0 if self.is_collision(agent, adv) else 0.0

                # 计算距离惩罚 (距离越小，惩罚越小)
                distance = np.linalg.norm(agent.state.p_pos - adv.state.p_pos)
                distance_penalty = distance / world.world_size  # 归一化

                # 累加到全局奖励
                global_rew += (collision_reward - distance_penalty)

            # 减去逃避者速度惩罚项 (速度越快惩罚越大)
            evader_speed = np.linalg.norm(agent.state.p_vel)

            global_rew -= evader_speed / agent.max_speed  # 归一化

            # 额外奖励：全部追捕者都接近逃避者时 # 此处需要再调整一下
            if all(np.sqrt(np.sum(np.square(agent.state.p_pos - adv.state.p_pos))) < capture_threshold for adv in adversaries):
    ...
    def observation(self, agent, world):  # 返回值，自动适配智能体的观测空间维数
        ...
        # 自身状态归一化
        norm_self_vel = agent.state.p_vel / agent.max_speed # 速度归一化到[-1,1]
        norm_self_pos = agent.state.p_pos / (world.world_size) # 位置归一化到[-1,1]
