...
class Custom_raw_env(SimpleEnv, EzPickle):
    def __init__(
        ...
        world_size = 1, # Ronchy添加 world_size参数, 地图大小: [-world_size , world_size]
    ):
        EzPickle.__init__(
            self,
            num_good=num_good,
        ...
        # Ronchy添加 world_size参数, 地图大小: [-world_size , world_size]
        self.world_size = world_size
        self.metadata["name"] = "simple_tag_env_v4_partially"
        # Ronchy添加轨迹记录
        ...
        # self.capture_threshold = self.world.world_size * 0.2 # 围捕阈值: 使用世界大小的20%作为默认捕获范围
        self.local_ratio = 0.5  # 全局奖励和局部奖励的比例
    ...
    #     cam_range = np.max(np.abs(np.array(all_poses)))
        ...
        for agent in self.scenario.good_agents(self.world):
            ...
            threshold_pixels = int(self.world.capture_threshold / cam_range * self.width // 2 * 0.9)  # 与实体渲染使用相同的缩放逻辑
...
class Scenario(BaseScenario):
    def make_world(self, num_good=1, num_adversaries=3, num_obstacles=2, _world_size = None):
        ...
        world.capture_threshold = _world_size * 0.15  # Ronchy添加围捕半径
        # V4环境特性
        world.observation_radius = _world_size * 0.2  # # 添加观测半径，半径是世界大小的30%
        ...
        # add agents
        world.agents = [Agent() for i in range(num_agents)]  # all_learning
        for i, agent in enumerate(world.agents):
            ...
            agent.size = base_size if agent.adversary else base_size*0.7  # 智能体的半径，判断是否碰撞的界定
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            landmark.size = base_size * 2.0  # 设置为基础大小的2倍
    ...
    def adversary_reward(self, agent, world):
        ...
        for target in agents:  # 通常只有一个逃跑者
            ...
            # 只有当逃跑者在观测范围内时，才给予距离相关奖励
            # if distance <= world.observation_radius:
            #     # 归一化距离系数
            ...
            #         approach_reward = 0.1 * -dist_factor
            #         rew += approach_reward
            #     else:
    ...
    # TODO：bug obs对reward的影响为0了，即使有一位追捕者观测到了逃跑者，其他从comm网络传去的信息没法体现在reward中，导致其他agent没有驱动力。
    ...
    def global_reward(self, world):
        ...
        # 安全地获取捕获信息
        distances, target_pos = [], None
        # 已有围捕信息，直接使用
        target = agents[0] # 获取第一个逃跑者
        target_pos = target.state.p_pos
        # target_speed = np.linalg.norm(target.state.p_vel)
        pursuers_can_observe = []
        for adv in adversaries:
            distance = np.linalg.norm(adv.state.p_pos - target_pos)
            distances.append(distance)
            pursuers_can_observe.append(distance <= world.observation_radius + target.size)
        if not any(pursuers_can_observe):
            return 0 # 这个后期用鼓励探索的内在激励
        
        # 只考虑能观测到目标的追捕者
        observable_distances = [d for d, can_observe in zip(distances, pursuers_can_observe) if can_observe]

        if len(observable_distances) > 0:
            # 计算平均距离和归一化
            mean_distance = np.mean(observable_distances) # 使用可观测距离的平均值
        else:
            mean_distance = 0
        ...
        # 时间压力删除
        time_pressure = -0.005 * world.episode_steps
        ...
        if agent.adversary:
            for other in world.agents: # 追捕者观察逃跑者信息
                if not other.adversary:  # 对于逃跑者
                    ...
                    if distance <= world.observation_radius + other.size: # 加入目标的半径
