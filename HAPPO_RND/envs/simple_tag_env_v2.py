...
class Custom_raw_env(SimpleEnv, EzPickle):
    ...
    def _is_point_surrounded(self, points, target):
        ...
        # # 方法1: 三角形面积法 - 适用于3个追捕者的情况
        # if len(points) == 3:
        #     # 计算三角形面积的函数
        #     def triangle_area(p1, p2, p3):
        #         return 0.5 * abs((p1[0]*(p2[1]-p3[1]) + p2[0]*(p3[1]-p1[1]) + p3[0]*(p1[1]-p2[1])))   
        #     # 计算追捕者形成的三角形面积
        #     S4 = triangle_area(points[0], points[1], points[2])
        #     # 如果三角形面积接近于0（三点共线），则无法形成有效包围
        #     if S4 < 1e-10:
        #         return False
        #     # 计算目标与每对追捕者形成的三个三角形面积
        #     S1 = triangle_area(target, points[0], points[1])
        #     S2 = triangle_area(target, points[1], points[2])
        #     S3 = triangle_area(target, points[2], points[0])
        #     # 计算三个小三角形面积之和
        #     sum_S = S1 + S2 + S3
        #     # 当且仅当目标在三角形内部时，sum_S == S4（允许小误差）
        #     return abs(sum_S - S4) < 1e-5
        ...
        if agent.adversary:
            for other in world.agents: # 追捕者观察逃跑者信息
                if not other.adversary:  # 对于逃跑者
                    relative_pos = (other.state.p_pos - agent.state.p_pos) / world.world_size # 相对位置
                    other_pos.append(relative_pos)
                    norm_vel = other.state.p_vel / other.max_speed# 归一化速度
