...
- 训练DDPG prey，给prey加入观测内的奖励鼓励逃跑。不影响random和stationary。
'''
...
class Custom_raw_env(SimpleEnv, EzPickle):
    def __init__(
        ...
        self.metadata["name"] = "simple_tag_env_all_learning_v5_Partially_2Phase"
        # Ronchy添加轨迹记录
    ...
    def agent_reward(self, agent, world):
        ...
        if (
            ...
            for adv in adversaries:
                ...
                if distance <= world.observation_radius + adv.size: # 加入智能体自身的半径
    ...
    def update_communication_phase(self, world): # 每一步都检查并更新通信阶段
        ...
        for target in agents: # 目前只有一个
            for adv in adversaries:
                ...
                if distance <= world.observation_radius + target.size: # 加入目标的半径
